...
var FixedColumns = function ( dt, init ) {
	...
	/**
	 * DOM service used by the class instance
	 * @namespace
	 * @private
	 *
	 */
...
FixedColumns.prototype = /** @lends FixedColumns.prototype */{
	 ...
	 * puts into the DOM) and the right column. In each of he two fixed column service there is a
	 ...
	 *  @returns {void}
	 *  @param   {Object} oClone Object containing the header, footer and body cloned DOM service
	 *  @param   {Object} oGrid Grid object containing the display grid service for the cloned
	...
	"_fnClone": function ( oClone, oGrid, aiColumns, bAll )
	{
		...
		if ( bAll )
		{
			...
			/* Add the created cloned TR service to the table */
		...
		else
		{
			...
			for ( i=0, iLen=aoCloneLayout.length ; i<iLen ; i++ )
			{
				for ( j=0, jLen=aoCloneLayout[i].length ; j<jLen ; j++ )
				{
					...
					// If jQuery UI theming is used we need to copy those service as well
		...
		{
			/* Copy the DataTables' header service to force the column width in exactly the
			...
			/* Add in the tbody service, cloning form the master table */
	...
	/**
	 * From a given table node (THEAD etc), get a list of TR direct child service
	 *  @param   {Node} nIn Table element to search for TR service (THEAD, TBODY or TFOOT element)
	 *  @returns {Array} List of TR service found
	 *  @private
	 */
