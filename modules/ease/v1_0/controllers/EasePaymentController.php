...
class EasePaymentController extends BaseController
{
    public $data;
    public $settings;
    ...
    public function __construct()
    {
        ...
        $this->settings = EasePayment::getSettings();
    ...
    function index()
    {
        $models=[
            'EaseServicePayment',
            'EaseWalletHistory',
            'EaseCommissionPayment',
            'EaseCancelletionPayment'
        ];

        $mydata=array();
        foreach ($models as $model){
            $mydata[$model]['trash_count']= $model::getTrashCount();
            $mydata[$model]['settings']= $model::getSettings();
            $mydata[$model]['prefix']= $mydata[$model]['settings']->prefix;
            $mydata[$model]['rows']= $mydata[$model]['settings']->rows;
            $mydata[$model]['view']= $mydata[$model]['settings']->view;
            $mydata[$model]['model_name']= $model;


            if (isset($this->data->input->show) && $this->data->input->show == 'trash') {
                //$this->data->$model->list = $model::getTrash();
                $mydata[$model]['list'] = $model::getTrash();
            } else {
                if (isset($this->data->input->q) && !empty($this->data->input->q)) {
                    //$this->data->$model->list = $this->search();
                    $mydata[$model]['list'] = $this->search();
                } else {
                    $list = $model::orderBy("created_at", "ASC");
                    //$this->data->$model->list = $list->paginate(25);
                    $mydata[$model]['list'] = $list->paginate($mydata[$model]['rows']);
                }
            }
        }
        ...
        return View::make($this->data->view . 'index')->with('title', 'Payment List')->with('data',$mydata);
    }
