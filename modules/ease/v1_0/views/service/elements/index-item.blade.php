<tr>
    ...
    <td>
        <?php
        $baseUrl=substr($item->icon_url, 0, strrpos($item->icon_url, '/') + 1);
        $fileName = basename($item->icon_url);
        ?>
        <a target="_blank" href="<?php echo $item->icon_url; ?>">
            <img src="<?php echo $baseUrl.'/'.$fileName ?>" alt="" />
        </a>
    </td>
    <td>
        <?php
        $baseUrl=substr($item->icon_url_circle, 0, strrpos($item->icon_url_circle, '/') + 1);
        $fileName = basename($item->icon_url_circle);
        ?>
        <a target="_blank" href="<?php echo $item->icon_url_circle; ?>">
            <img src="<?php echo $baseUrl.'/'.$fileName ?>" alt="" />
        </a>
    </td>
    @if($item->unit == "session")
        <td>session</td>
    @endif
    @if($item->unit=="time")
        <td>time</td>
